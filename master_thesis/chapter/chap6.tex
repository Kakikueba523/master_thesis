...
\begin{align}
  \bm{H}^l_{\text{C+C}, ij} = -\frac{\hbar^2}{2\mu}\nabla^2 + V_l(r)
\end{align}
となる．
...
断面積の計算は，$0.5$MeV$<E_{\mathrm{c.m.}} < 5.5$ MeVで行った．

また，空間メッシュは$\Delta r = 0.01$fmであり，ボックスの大きさは，30fmとした．
KSHELLを用いた殻模型計算は，京都大学基礎物理学研究所の
大型計算機システムYukawa-21を用いて実行した．
その他全ての計算は，手元のノートパソコンでプログラミング言語Julia（version 1.11.6）を用いて実行した．
