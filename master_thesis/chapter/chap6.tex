\chapter{模型}\label{chap:model_explanation}
...
また，文献\cite{PhysRevC.98.014604}では複合核状態をランダム行列で記述していたが，本研究では
複合核状態をより微視的原子核理論に基づいて与える．
\begin{figure}[tbp]
    \centering
    \begin{tikzpicture}[>=Stealth]
        ...
        % 各座標の点とラベル (r_ie-2 から r_ie+3)
        \foreach \x/\label/\name/\col/\rad in {
            1/{i_{e-2}}/r0/blue/0.15,
            3/{i_{e-1}}/r1/blue/0.15,
            5/{i_{e}}/r2/red/0.2,
            7/{i_{e+1}}/r3/blue/0.15,
            9/{i_{e+2}}/r4/blue/0.15,
            11/{i_{e+3}}/r5/blue/0.15
        } {
            \fill[\col] (\x,0) circle (\rad);
            \node[below, yshift=-2pt] at (\x,0) {$r_{\label}$};
        }

        % --- r軸の矢印 ---
        \draw[->, thick] (2,1.5) -- (10,1.5) node[right, xshift=5pt] {$r$};

        % --- 中央の垂直構造 (x=5) ---
        % v0 部分
        \draw[->, line width=5pt] (5,-1) -- (5,-3);
        \node[right, font=\large] at (6,-2) {$\bm{V}$};

        % 崩壊部分
        \draw[->, line width=5pt] (5,-5) -- (5,-6);
        \node[right, font=\large] at (7.6,-5.5) {$\Gamma(p, n, \alpha \text{ decay})$};

        % --- 複合成核 (CN) の円 ---
        \draw[green, thick] (5,-4) circle (1.2cm);
        \node[draw=none, fill=white, inner sep=2pt] at (5,-4) {\textbf{CN}};

    \end{tikzpicture}
    \caption{
      本研究で用いるハミルトニアンの概念図．
      \isotope{C}+\isotope{C}チャネルは
      $r = r_{i_e}$でのみ複合核と結合しており，
      その結合は$\bm{V}$により決められる．
    }
...
図~\ref{fig:model_schematic}にハミルトニアン~\eqref{eq:total_hamiltonian}
の概念図を書いた．

\section{入口チャネル}
  $\bm{H}_\text{C+C}$は，
炭素原子核$\isotope[12]{C}$，$\isotope[13]{C}$を球形と近似し，二つの原子核間の相対運動を記述する．
二つの原子核間の重心間距離を$\bm{r}$とすると，動径方向のみを考えればよいので，ハミルトニアンは
\begin{align}
  \bm{H}^l_{\text{C+C}} = -\frac{\hbar^2}{2\mu}\nabla^2 + V_l(r)
\end{align}
となる．
$V_l(r)$は遠心力ポテンシャルを含む原子核間ポテンシャルであり，原子核間ポテンシャルとして
M3Y + repulsionポテンシャルを用いる．
数値計算では，このハミルトニアンを動径方向$r$に関して等間隔メッシュ幅$\Delta r$で離散化し，
差分近似により取り扱う．また，境界を$r_{\mathrm{max}} = N_{\text{C+C}}\Delta r$とする．
これによりハミルトニアンは，$N_{\text{C+C}} \times N_{\text{C+C}}$行列として
...
$r_i = i \Delta r$と書いた．
なお，ここで$\isotope[12]{C}$，$\isotope[13]{C}$の内部励起は考えず，質点として取り扱っている．
...
また，崩壊幅は\isotope[24]{Mg}および\isotope[25]{Mg}ともに，遷移状態理論と呼ばれる
統計模型を用いて見積もる．
...
\end{align}
という形を用いる．図~\ref{fig:coupling_matrix}に概念図を書いた．
ここで，$\Delta r$はメッシュ間隔であり，$i_e$は相互作用点である．
複合核状態のラベル$\mu$には寄らず，すべての状態と同じ強さ$v_0$で結合することが分かる．
...
\begin{figure}[tbp]
    ...
    thick,
    font=\small,
    state/.style={anchor=west},
    note/.style={font=\footnotesize, align=left},
  ]

    % ---- P space (entrance channel) ----
    % \node[anchor=west] at (-5.2,4.0) {};
    \node[anchor=west] at (-3.2,3.45) {$\mathrm{C}+\mathrm{C}$};

    % radial axis
    \draw[->] (-5.2,2) -- (0,2) node[above] {$r$};

    % wave amplitude curve (schematic)
    % \draw[very thick]
    %   (-5.0,0.8) .. controls (-4.2,1.15) and (-3.6,0.35) .. (-3.0,0.55)
    %   .. controls (-2.4,0.75) and (-1.8,1.05) .. (-1.2,0.7)
    %   .. controls (-0.9,0.55) and (-0.6,0.85) .. (-0.35,0.75);
    % \node[anchor=west] at (-4.9,1.10) {$\psi_P(r)$};

    % coupling point rc
    \coordinate (rc) at (-2.0,2.0);
    % \draw[densely dotted] (rc) -- ++(0,1.55);
    \fill (rc) circle (2.2pt);
    \node[above] at (rc) {$r=r_{i_e}$};
    % \node[note, anchor=west] at (-5.2,-1.2)
    %   {$V_{P\mu}(r)=v_0\,\delta(r-r_c)$\\
    %   (mesh: $V_{i_e,\mu}=v_0(\Delta r)^{-1/2}$)};

    % ---- Q space (compound states) ----
    \node[anchor=south] at (2.29,2.0) {$\mathrm{Mg}$};

    % Q-box
    % \draw[rounded corners=2pt] (0.6,-1.6) rectangle (5.6,1.6);

    % energy axis inside Q-box
    % \draw[->] (1.0,-1.35) -- (1.0,1.35) node[above] {$E$};

    % levels (ladder)
    \foreach \y/\lab in {1.05/{|\mu=1\rangle,\ (E_1,J_1)},
                        0.45/{|\mu=2\rangle,\ (E_2,J_2)},
                       -0.15/{|\mu=3\rangle,\ (E_3,J_3)}}
    {
      \draw (1.25,\y) -- (3.3,\y);
      \node[state] (q\y) at (3.45,\y) {$\lab$};
    }
    \node at (4.58/2,-0.55) {$\vdots$};
    \draw (1.25,-1.05) -- (3.3,-1.05);
    \node[state] (qbot) at (3.45,-1.05) {$|\mu\rangle,\ (E_\mu,J_\mu)\ $};

    % ---- Couplings: show a few arrows + brace "for all mu" ----
    \draw[->, dashed] (rc) -- (1.25, 1.05) node[midway, above] {$v_0$};
    \draw[->, dashed] (rc) -- (1.25, 0.45) ;
    \draw[->, dashed] (rc) -- (1.25,-0.15)  ;
    \draw[->, dashed] (rc) -- (1.25,-1.05)  ;

    % brace indicating "all states"
    \draw[decorate, decoration={brace, amplitude=3pt}]
      (7.25,1.15) -- (7.25,-1.15)
      node[midway, right, note] {すべての状態と同じ$v_0$で結合};

    % Optional label: one-point coupling
    % \node[note, anchor=west] at (0.8,-1.45)
    %   {one-point coupling at $r_c$};

  \end{tikzpicture}
    \caption{
      本研究で用いるカップリング行列$\bm{V}$の概念図．
    }
...
\begin{align}
  ...
  u(r) &\rightarrow A(k) \qty[H_l^- (kr) - S^{J^\pi}_l H_l^+(kr)]\qquad (r\rightarrow\infty) \label{eq:boundary_approxfrom}
\end{align}
...
次に，シュレーディンガー方程式について考える．式~\eqref{eq:entrance_hamiltonian}からわかるように，$\isotope{C}+\isotope{C}$チャネルにおいて
...
\begin{align}\label{eq:fus_cs_12_12}
  \sigma_{\text{fus}, \isotope[12]{C} + \isotope[12]{C}} = \frac{\pi}{k^2} \sum_{l:\mathrm{even}} (2l + 1) \qty(1-\abs{S^{J^\pi=l^+}_l}^2) \qty(1 + (-)^l)
\end{align}
となる．
...
$S$行列の絶対値は1にとても近い量であるため，直接計算すると精度が足りない．
...
\isotope[24]{Mg}は，$E_{\mathrm{c.m.}} \sim 8$ MeVに対応するエネルギーまで，励起スペクトルを計算した．
\isotope[25]{Mg}は，$E_{\mathrm{c.m.}} \sim 6$ MeVかつ相対角運動量が$ l_{\mathrm{max}} = 5$に対応する
エネルギーまで，励起スペクトルを計算した．
崩壊幅の計算が$E_{\mathrm{c.m.}} \gtrsim 0.5$MeVで計算できたことと併せて，
断面積の計算は，$0.5$ MeV $<E_{\mathrm{c.m.}} < 5.5$ MeV で行った．

また，空間メッシュは$\Delta r = 0.01$fmであり，ボックスの大きさは，30 fm とした．
KSHELLを用いた殻模型計算は，京都大学基礎物理学研究所の
大型計算機システムYukawa-21を用いて実行した．
その他全ての計算は，自前のノートパソコンでプログラミング言語Julia（version 1.11.6）を用いて実行した．
