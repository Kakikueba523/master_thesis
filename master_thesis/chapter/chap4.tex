\chapter{殻模型計算}\label{chap:shell_model}

本研究では，\isotope{C}+\isotope{C}反応でできる複合核(\isotope{Mg})の励起状態をKSHELLという計算コード\cite{SHIMIZU2019372}により求める．
...
生成演算子 $c^\dagger_{i, 1}$ は具体的に書くと
主量子数$n$，全角運動量$J$，軌道角運動量$l$，全角運動量の$z$成分$m$を量子数に持つ軌道の
生成演算子であるため，
$c^\dagger_{njlm}$と書かれる．


式~\eqref{eq:m_scheme_basis}を用いて生成される$M$-scheme基底の一例を図\ref{fig:m_basis_shcematic_fig}に書いた．
これらを並べて書いた$(a_{i,1}, a_{i,2}, \dots, a_{i,A})$は配位と呼ばれ，コンピュータで表現するのに便利である．
また，$M$-scheme基底で表される状態は，全角運動量$J$の固有状態になっていない．
全角運動量の$z$成分およびパリティが固有状態であり，全角運動量の$z$成分は
\begin{align}
  M = \sum_i m_i
\end{align}
のように書かれる．
...
\end{tikzpicture}
\caption{
  $M$-shceme基底の一例．白丸が空孔を，黒丸が粒子を表している．
...
\section{実際の計算結果}
本研究では\isotope[16]{O}を不活性コアとし，
$sd$シェルおよび$pf$シェルをバレンス軌道とした
相互作用$sdpf$-muを用いた\cite{PhysRevC.86.051301}．
\isotope[24]{Mg}と\isotope[25]{Mg}
の積算状態数を図~\ref{fig:sekisan_state}に示す．
ただし，\isotope[24]{Mg}ではスピンおよびパリティに
制限はないものの，\isotope[25]{Mg}では
正パリティでは$J = 11/2$，負パリティでは$J = 9/2$
以下のスピンの状態を計算した．
また，参照として\isotope[12]{C}+\isotope[12]{C}核融合反応で
実際に現れる\isotope[24]{Mg}の偶数スピンおよび正パリティの
積算状態数をプロットした．
同じ励起エネルギーにおいて，\isotope[24]{Mg}と\isotope[25]{Mg}
の積算状態数は一桁程度の違いがあることがわかる．

\begin{figure}[tb]
  \centering
  \includegraphics[width=0.9\textwidth]{figure/chap4/total_nos_24mg_25mg.png}
  \caption{
    KSHELLによる計算結果から求めた，累積状態数の比較．
    \isotope[24]{Mg}が青線で書かれており，
    \isotope[25]{Mg}が赤線で書かれている．
    また，\isotope[24]{Mg}のスピンが偶数，パリティが正の状態数を水色でプロットした．
    \isotope[25]{Mg}は正パリティは $J = 11/2$，負パリティは $J = 9/2$ 以下のスピンを
    計算した． 
  }
  \label{fig:sekisan_state}
\end{figure}
