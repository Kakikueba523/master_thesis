...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    >=Stealth,
    thick,
    nuc/.style={circle, draw, minimum size=18mm, align=center},
    part/.style={circle, draw, minimum size=3mm, align=center},
  ]

    % --- nodes ---
    \node[nuc] (C) {複合核\\[-1mm]$E_C,\;J$};

    % split point (optional, makes the branching look nice)
    \coordinate (V) at ($(C.east)+(2.4,0)$);

    \node[part, minimum size=7mm] (y) at ($(V)+(3.6, 2.0)$) {$y$};

    \node[nuc]  (B) at ($(V)+(2.6,-0.4)$) {残留核\\[-1mm]$B,\;I_B$};

    % --- arrows ---
    \draw[->] ($(C.east)+(0.4, 0)$) -- (V);
    % \draw[->] (V) -- (y.west);
    % \draw[->] (V) -- (B.west);
    \draw[<->] (y) -- node[midway, right] {$\mathcal{E}_b +\dd{\mathcal{E}_b}$} (B);

  \end{tikzpicture}
  \caption{複合核の崩壊過程の模式図．
  複合核 $(E_C,J)$ が蒸発粒子 $y$ と残留核 $B$ に崩壊する。}
  \label{fig:compound_decay}
\end{figure}

複合核の崩壊について考える．図~\ref{fig:compound_decay}のようにエネルギー$E_C$，スピン$J$を持つ複合核状態$C$
