\chapter{散乱理論}\label{chap:scattering_theory}
本研究において，重要なパートを担う散乱理論についてこの節で議論する．
クーロン場がない場合から始め，次にそれがある場合について述べる．
また，同種粒子同士の散乱問題についても，相互作用がスピンに依らない場合に限り，議論する．
なお，本章における議論は\cite{canto2013scattering}，\cite{1970586434905966474}，\cite{fröbrich1996theory}を参考にした．
...
\begin{figure}[thpb]
  \centering
  \includegraphics[width=0.9\textwidth]{figure/chap3/potential_example.pdf}
  \caption{原子核間ポテンシャルの相対角運動量が0の場合の例．
  赤の点線がクーロンポテンシャルであり，青の破線が核力に由来するポテンシャルである．
...
短距離力である核力と，長距離力であるクーロンポテンシャルの競合により，ポテンシャル障壁が
...
式~(\ref{eq:stationary_schrodinger_eq})の解は，入射平面波と散乱波の二つに分解できて，
...
\end{align}
であり，$\psi^{(+)}(\bm{k};\bm{r})$は散乱源により生じた外向きの球面波である．よって，$r \rightarrow \infty$において，
\begin{align}\label{eq:scatt_asympt_form}
  \psi^{\text{sc}}(\bm{k};\bm{r}) \rightarrow A f(\Omega) \frac{e^{ikr}}{r}
\end{align}
のような漸近的な振る舞いをする\footnote{理論的な定式化を付録\ref{app:sa}で行った．}．
...
微分散乱断面積は入射ビームのフラックスと，図\ref{fig:scattering_multi}にあるように，立体角$\Omega$に散乱された粒子のフラックスの比で与えられる．
...
式~(\ref{eq:radial_eq_by_rho})は球ベッセル方程式に帰着される．その解は，独立な二つの解$j_l(\rho)$と$n_l(\rho)$の
...
\end{align}
である．
また，$\rho \rightarrow 0$での振る舞いは，
\begin{align}
  j_l(\rho \rightarrow 0) \sim \frac{\rho^l}{(2l+1)!!} \\
  n_l(\rho \rightarrow 0) \sim \frac{(2l-1)!!}{\rho^{l+1}}
\end{align}
である．
複素数の解として，
...
\end{align}
であり，$\rho \rightarrow 0$での振る舞いは，
\begin{align}
  \hat{\jmath}_l(\rho \rightarrow 0) \sim \frac{\rho^{l+1}}{(2l+1)!!} \\
  \hat{n}_l (\rho\rightarrow 0) \sim \frac{(2l -1)!!}{\rho^{l}}
\end{align}
である．
...
$r = R$より大きな$r$では，0と近似できるポテンシャル$V(r)$について考える．
ここで，$r \le R$の領域を領域Ⅰ，$r > R$の領域を領域Ⅱとする．
...
\end{align}
となる．ここで，関数の引数，つまり$kr$での微分をプライムを用いて表している．
...
と同じ$kr - l\pi/2$という位相を持つ．$\delta_l$は位相差と呼ばれポテンシャル$V$が生じさせる．
...
\begin{align}\label{eq:outgoing_base_expansion}
  \psi^{(+)}(\bm{k};\bm{r}) = \sum_l C_l P_l(\cos\theta) \frac{u_l(k, r)}{kr}
\end{align}
と展開することができる．$r \rightarrow \infty$で，式~(\ref{eq:phase_shift_hankel})を用いることで，
\begin{align}\label{eq:outoing_partial_wave_expansion}
  \psi^{(+)}(\bm{k}; \bm{r}) \rightarrow \sum_{l=0}^{\infty} \gamma_l C_l P_l(\cos\theta) \frac{e^{-i(kr-l\pi/2)} -S_l e^{i(ir - l\pi/2)}}{kr} = \frac{e^{-ikr}}{r} X_{-} + \frac{e^{ikr}}{r} X_{+}
\end{align}
となる．ここで，
...
\begin{align}\label{eq:phi_pwe}
  \phi(\bm{k};\bm{r}) = A \sum_l (2l+1)P_l(\cos\theta) i^l \frac{\hat{\jmath}_l(kr)}{kr}
\end{align}
$\hat{\jmath}_l(kr)$の漸近形を用いて，$\sin(kr-l\pi/2)$を指数関数で書くことにより，
\begin{align}\label{eq:plane_wave_approx_form}
  e^{ikr\cos\theta} \rightarrow \frac{1}{2i} \sum_{l=0}^{\infty} (2l+1)i^l P_l(\cos\theta) \qty[\frac{e^{i(kr-l\pi/2) } - e^{-i(kr-l\pi/2)}}{kr}]
\end{align}
という漸近形が得られる．
...
\begin{align}\label{eq:outgoing_incoming_decomposition}
  \psi^{(+)}(\bm{k};\bm{r}) \rightarrow \frac{e^{-ikr}}{r} \bar{X}_- + \frac{e^{ikr}}{r} \bar{X}_+
\end{align}
となる．ここで，
\begin{align}
...
\begin{align}
  f(\theta) 
  &= \frac{1}{2ik}\sum_l P_l(\cos\theta) (2l+1)\qty[S_l -1] \label{eq:sa_pwe} \\
...
弾性散乱断面積は式~\eqref{eq:diff_cross_section_by_scattering_amplitude}から
\begin{align}
  \dv{\sigma}{\Omega} = \abs{f(\theta)}^2
\end{align}
で得られるので，ルジャンドル多項式の直交性
\begin{align}
...
\end{align}
を得る．
式~(\ref{eq:scattering_amplitude_partial_expansion})から，
\begin{align}
...
断面積に反映されていることを表している．
\subsection{部分波展開の収束について}
短距離ポテンシャルが$r = R$より大きな$r$では0とみなせるような場合を考える．
遠心力ポテンシャルは
\(
  l(l+1)/r^2
\)
で書かれる．短距離ポテンシャルを感じるためには$r < R$へと粒子が侵入しなくてはならない．
つまり，古典的には波数$k$に対して，
\begin{align}
  \frac{l(l+1)}{R^2} < k^2
\end{align}
が成り立つような$l$のみが，位相差$\delta_l$を持つ．
ここで，その最大の$l$，つまり
\begin{align}
  \frac{l(l+1)}{R^2} = k^2
\end{align}
が成り立つ$l$を$l_{\mathrm{cut}}$と置き，
$l_{\mathrm{cut}}(l_{\mathrm{cut}} + 1) \approx l_{\mathrm{cut}}^2$
と置き換えることで，
\begin{align}
  l_{\mathrm{cut}} = kR
\end{align}
...
\begin{align}\label{eq:absorption_cross_section_of_ev}
  \sigma_a = \frac{k}{\abs{A}^2E_k}\ev{W}{\psi^{(+)}(\bm{k})}
\end{align}
と書くことができる．
...
\begin{align}
  j_r &= \frac{\hbar}{2i \mu} \qty[\qty(\psi^{(+)}(\bm{k};\bm{r}))^* \pdv{r} \qty(\psi^{(+)}(\bm{k};\bm{r})) - \qty(\psi^{(+)}(\bm{k};\bm{r}))\pdv{r} \qty(\psi^{(+)}(\bm{k};\bm{r}))^*] \notag\\
   &= \frac{\hbar}{\mu} \mathrm{Im}\qty[\qty(\psi^{(+)}(\bm{k};\bm{r}))^*\pdv{r} \qty(\psi(\bm{k}; \bm{r})^{(+)})]
\end{align}
となる．
...
原子核反応の計算が行われてきた．
ここでは，Feshbachの方法\cite{FESHBACH1967230}に従って複素ポテンシャルの理論的な導出を考える．
核融合反応過程では，始状態の二つの原子核が複合核と呼ばれる中間状態を経て
...
そうでないときは，入射波動関数がポテンシャルによって歪められる．
荷電粒子同士の反応におけるクーロン力がその例である．
...
ここで，$\xi$が定数の曲面は，原点を共通の焦点としての$z$の方向に開いた放物線を$z$軸回りに
回転してできる回転放物面の集まりである．
また，$\zeta$が定数の曲面も同様に，$z$の負の方向に開いた回転放物面の集まりである．
これは，式~(\ref{eq:def_of_parabolic_coordinates})の形から明らかである．
...
\end{align}
であり，$e^{-ikr}$の形を含んでいない．
このことから$e^{ikz}$という位相因子を取り出したときに，
$g(r+z)$ではなくて$g(r-z)$となることが予想される．
...
\end{align}
である．$F(-i\eta, 1, ik(r-z))$はラプラス型の方程式の特殊解$W_1，W_2$
の和で表せて，$|z| \rightarrow \infty$での漸近形が知られている\cite{Messiah1961QMv1}．
$C = A \Gamma(1 + i\eta)e^{\pi\eta/2}$とすることで，
\begin{align}\label{eq:coulomb_sol_asym}
  \phi_C(\bm{k};\bm{r}) \sim A\qty(e^{i(kz - \eta \ln k(r-z))} +  f_C(\theta) \frac{e^{ikr}}{r}\qty[e^{-i\eta \ln(2kr)}])
\end{align}
と漸近解が得られる．$A$は規格化定数である．
ただし，$f_C(\theta)$はクーロン散乱振幅であり，
\begin{align}\label{eq:pure_c_sa}
  f_C(\theta) = -\frac{\eta}{2k\sin^2(\theta/2)}e^{-i\eta\ln(\sin^2\theta/2)}e^{2i\sigma_0}
\end{align}
である．ここで，$\sigma_0$は$l = 0$のクーロン位相差であり，
\begin{align}
...
クーロン波動関数$\phi_C(\bm{k};\bm{r})$を部分波展開すると，
\begin{align}\label{eq:coulomb_partial_exp}
  \phi_C(\bm{k};\bm{r}) = \frac{1}{(2\pi)^{3/2}}\sum_{l,m} C_{lm}Y_{lm}(\hat{\bm{r}}) \frac{u_{C,l }(k, r)}{kr}
\end{align}
となり，$u_{C,l}$についてのシュレーディンガー方程式は，
\begin{align}
  -\frac{\hbar^2}{2\mu}\qty[\dv[2]{r} - \frac{2\eta k}{r} - \frac{l(l+1)}{r^2}]u_{C, l}(k, r) = E u_{C, l}(k, r)
\end{align}
と書かれる．
$r \rightarrow \rho = kr$と変数変換することにより，
\begin{align}
  u_{C, l}''(\rho) + \qty[1 - \frac{l(l+1)}{\rho^2} - \frac{2\eta }{\rho}]u_{C, l}(\rho) = 0
\end{align}
を得る．この方程式の解は二つの独立な解の線形結合としてあらわされる．
一つは，原点で正則な解$F_l(\eta, \rho)$であり，もう一つは$l=0$を除き，原点で非正則な解$G_l(\eta, \rho)$である．
この二つの解の性質を以下にまとめよう．
\begin{itemize}
  \item $\eta \rightarrow 0$において，ベッセル関数となる．
    \begin{align}
      &F_l(\eta \rightarrow 0, \rho) \rightarrow \hat{\jmath}_l(\rho) \\
      &G_l(\eta \rightarrow 0, \rho) \rightarrow \hat{n}_l(\rho) 
    \end{align}
  \item $\rho \rightarrow 0$における振る舞い．
  \begin{align}
    &F_l(\eta, \rho \rightarrow 0) \rightarrow A_l \rho^{l+1} \\
    &G_l(\eta, \rho \rightarrow 0) \rightarrow \frac{1}{(2l+1)A_l}\rho^{-l}
  \end{align}
  ここで，
  \begin{align}
    A_0 &= \qty(\frac{2\pi \eta}{e^{2\pi \eta}-1})^{1/2} \\
    A_{l>0} &= \frac{A_0}{\qty(2l+1)!!}\prod_{m=1}^{l}\qty(1+\frac{\eta^2}{m^2})^{1/2}
  \end{align}
  \item $\rho \rightarrow \infty$における振る舞い．
  \begin{align}
    F_l(\eta, \rho \rightarrow \infty) &\rightarrow \sin \qty[\rho- \frac{l\pi}{2}-\eta \ln(2\rho) + \sigma_l] \label{eq:regular_coulomb_asympt} \\
    G_l(\eta, \rho \rightarrow \infty) &\rightarrow \cos \qty[\rho- \frac{l\pi}{2}-\eta \ln(2\rho) + \sigma_l] \label{eq:irregular_coulomb_asympt}
  \end{align}
\end{itemize}
式~(\ref{eq:regular_coulomb_asympt})と~(\ref{eq:irregular_coulomb_asympt})
において，$l$部分波におけるクーロン位相差
\begin{align}\label{eq:coulomb_ps}
  \sigma_l = \frac{1}{2i}\ln\qty(\frac{\Gamma(l+1+i\eta)}{\Gamma(l+1-i\eta)}) =\arg\qty[\Gamma(1+l + i\eta)]
\end{align}
を導入した．
クーロン波動関数は外向き波と内向き波として表すこともできる．それぞれを$H_l^{(+)}，H_l^{(-)}$と書くと，
\begin{align}
  H_l^{(\pm)}(\eta, \rho) = G_l(\eta, \rho) \pm i F_l(\eta, \rho)
\end{align}
となる．
これらの漸近形は，
\begin{align}\label{eq:coulomb_wave_asympt}
  H_l^{(\pm)} (\eta, \rho \rightarrow \infty) \rightarrow e^{\pm i \qty[\rho - \frac{l\pi}{2} - \eta \ln(2\rho) + \sigma_l]}
\end{align}
である．クーロン場が弱い極限$\eta \rightarrow 0$においては，
$H_l^{(\pm)}$はリッカチハンケル関数へと収束する
\begin{align}
  H_l^{(\pm)}(\eta \rightarrow 0, \rho) \rightarrow \hat{h}_l^{(\pm)}(\rho)．
\end{align}

式~\eqref{eq:coulomb_partial_exp}は
\begin{align}
  u_{C, l} (k, r) = F_l(\eta, \rho)
\end{align}
としたもので与えられ，係数は
\begin{align}\label{eq:n_c_for_c}
  C_{lm} = 4\pi Y_{lm}^*(\hat{\bm{k}}) i^l e^{i \sigma_l}
\end{align}
である\cite{Messiah1961QMv1}．
つまり，
\begin{align}
  \phi_C(\bm{k};\bm{r}) &= \frac{1}{(2\pi)^{3/2}} \sum_{lm} 4\pi Y_{lm}^*(\hat{\bm{k}}) Y_{lm}(\hat{\bm{r}}) i^l e^{i\sigma_l}\frac{F_l(\eta, \rho)}{kr} \\
  &=\frac{1}{(2\pi)^{3/2}}\sum_l (2l+1) P_l(\cos\theta) i^l e^{i\sigma_l} \frac{F_l(\eta, \rho)}{kr}
\end{align}
となる．また，$\eta \rightarrow 0$ の極限において，
$\sigma_l \rightarrow 0$，$F_l(\eta, \rho) \rightarrow \hat{\jmath}(\rho)$
となるため，上式が式~\eqref{eq:phi_pwe}に収束することがわかる．
ただし，ここでは規格化定数として
$
  A = 1 /(2\pi)^{3/2}
$
を採用した．
また，外向きクーロン波と内向きクーロン波で書き直すと，
\begin{align}
  \phi_C(\bm{k};\bm{r}) &= \frac{1}{(2\pi)^{3/2}} \sum_{lm} 4\pi Y_{lm}^*(\hat{\bm{k}}) Y_{lm}(\hat{\bm{r}}) i^l e^{i\sigma_l}\times\frac{i}{2}\qty[\frac{H_l^{(-)}(\eta, \rho) - H_l^{(+)}(\eta, \rho)}{kr}] \label{eq:coulomb_sol_aym} \\
  &=\frac{1}{(2\pi)^{3/2}}\sum_l (2l+1) P_l(\cos\theta) i^l e^{i\sigma_l} \times \frac{i}{2}\qty[\frac{H_l^{(-)}(\eta, \rho) - H_l^{(+)}(\eta, \rho)}{kr}]
\end{align}
となる．$r\rightarrow\infty$の極限で，式~\eqref{eq:coulomb_wave_asympt}を用いることで，
\begin{align}\label{eq:pure_c_asym}
  \phi_C(\bm{k};\bm{r}) \rightarrow \frac{1}{(2\pi)^{3/2}}\qty[\frac{e^{-i\Theta(r)}}{r}Z_- + \frac{e^{i\Theta(r)}}{r}Z_+]
\end{align}
を得る．ここで，
\begin{align}
  Z_- = -\frac{1}{2ik}\sum_{lm} 4\pi Y_{lm}^*(\hat{\bm{k}}) Y_{lm}(\hat{\bm{r}}) i^{2l}
\end{align}
および，
\begin{align}
  Z_+ = \frac{1}{2ik}\sum_{lm} 4\pi Y_{lm}^*(\hat{\bm{k}}) Y_{lm}(\hat{\bm{r}}) e^{2i\sigma_l}
\end{align}
とした．また，
\begin{align}\label{eq:coulomb_aympt_phase}
  \Theta(r) = kr - \eta \ln 2kr
\end{align}
である．

\subsection{部分波展開の収束について}
クーロン位相差は$l$を大きくしていくとむしろ増大し，0にならない．
$l$を十分に大きくとり，
式~\eqref{eq:coulomb_ps}でスターリングの公式を用いると，
\begin{align}
  \sigma_l 
  &= \mathrm{Im} \qty( \ln\Gamma(l+1 + i\eta)) \notag \\
  &\rightarrow \mathrm{Im} \qty[(l+\frac{1}{2}+i\eta)\ln(l+1+i\eta) - i\eta] \rightarrow \eta \ln l
\end{align}
となる．これは，$ \rightarrow \infty$においてクーロンポテンシャルが
全ての$l$について遠心力ポテンシャルよりも大きな値をとるためであると理解できる．
このことから，
部分波展開から求めたクーロン散乱振幅
\begin{align}
  f_C(\theta) = \frac{1}{2ik} \sum_l (2l +1)P_l(\cos\theta) \qty(e^{2i\sigma_l} - 1)
\end{align}
は収束しない．

しかし，式~\eqref{eq:pure_c_sa}ではあらゆる$r$で収束した形が得られている．
部分波展開が収束しなかったのは$r \rightarrow \infty$の極限を先にとり，そのあと$l$についての和をとったことに起因する．

\section{クーロン＋短距離ポテンシャル}
現実の原子核同士の相互作用を記述するためには，核力に由来する短距離ポテンシャルだけでなく，
原子核が電荷をもつことに由来するクーロンポテンシャルも考慮しなくてはならない．
そこで，
ポテンシャルが，
\begin{align}
  V(r) = V_C(r) + V_0(r)
\end{align}
で与えられたときのポテンシャル散乱を考える．
ここで，$V_C(r)$はクーロンポテンシャルであり，$V_0(r)$は $r > R$で 0
とみなせる短距離ポテンシャルである．
この場合も$r$を大きくとっても $1/r$ で振る舞うポテンシャルが残るため前節で議論した部分波展開の収束の問題が生じる．しかし，クーロンポテンシャル下での解は閉じた形で記述できるため，波動関数からクーロンのみの寄与を分離し，残りを部分波で取り扱うことができる．

シュレーディンガー方程式は，
\begin{align}\label{eq:scheq_nucl_plus_coulomb}
  \qty[\nabla^2 + k^2 - \frac{2\eta k}{r}\psi(\bm{k};\bm{r})] = \frac{2\mu}{\hbar^2}V(r) \psi(\bm{k};\bm{r})
\end{align}
と書ける．クーロンポテンシャルがない場合と同様に，
\begin{align}\label{eq:c_plus_n_aym}
  \psi^{(+)}(\bm{k};\bm{r}) = \phi_C(\bm{k};\bm{r}) + \bar{\psi}^{\mathrm{sc}}(\bm{k}; \bm{r})
\end{align}
の形の解を探す．ここで，
\(
  \phi_C(\bm{k};\bm{r})
...
\(
  \psi^{\mathrm{sc}}
\)
はポテンシャル$V_0(r)$の影響を受けた外向きの球面波である．
しかし，クーロンポテンシャルの存在により漸近形が歪められて，
\begin{align}\label{eq:c_plus_n_f}
  \bar{\psi}^{\mathrm{sc}}(\bm{k};\bm{r}) _{\abs{r-z}\rightarrow \infty} \rightarrow \frac{1}{(2\pi)^{3/2}}\qty[\bar{f}(\theta)\frac{e^{i\Theta(r)}}{r}]
\end{align}
になる．$\Theta(r)$は式~\eqref{eq:coulomb_aympt_phase}で定義され位相であり，
$\bar{f}(\theta)$は短距離ポテンシャル$V_0(r)$の存在により生じるクーロン散乱振幅の補正である．

$\bar{f}(\theta)$を求めるために部分波展開，
\begin{align}
  \psi^{(+)}(\bm{k};\bm{r}) = \frac{1}{(2\pi)^{3/2}} \sum_{lm}C_{lm}Y_{lm}(\hat{\bm{r}}) \frac{u_l(k, r)}{kr}
\end{align}
の動径波動関数$u_l(k, r)$が満たす方程式は
\begin{align}\label{eq:c_plus_n_radial}
  -\frac{\hbar^2}{2\mu}\qty[\dv[2]{r} - \frac{l(l+1)}{r^2} - \frac{2\eta k }{r}] u_l(k, r) + V_0(r)u_l(k,r) = E u_l(k,r)
\end{align}
となる．$r > R$において，$V_0(r)$は0とできるため，式~\eqref{eq:c_plus_n_radial}
はクーロンポテンシャルのみの方程式に帰着される．
よって，$u_l(k, r > R)$は$H_l^{(+)}(\eta, \rho)$と$H_l^{(-)}(\eta, \rho)$の線形結合で表される．
規格化定数を$i/2$とすると，
\begin{align}\label{eq:c_plus_n_outside}
  u_l(k, r>R) = \frac{i}{2}\qty[H_l^{(-)}(\eta, \rho) - \bar{S}_l H_l^{(+)}(\eta,\rho)]
\end{align}
と書ける．
ここで，$\bar{S}_l = e^{2i\bar{\delta}_l}$は第$l$部分波の$V_0(r)$による$S$行列であり，
$\bar{\delta}_l$はその位相差である．$C_{lm}$は式~\eqref{eq:n_c_for_c}で定義されている．
式~\eqref{eq:c_plus_n_outside}から，
\begin{align}
  \psi^{(+)}(\bm{k};\bm{r}) \rightarrow \frac{1}{(2\pi)^{3/2}}\qty[\frac{e^{-i\Theta(r)}}{r}Z_- + \frac{e^{i\Theta(r)}}{r}\bar{Z}_+]
\end{align}
と書かれる．ここで，
\begin{align}
  \bar{Z}_+ = \frac{1}{2ik}\sum_{lm} 4\pi Y_{lm}^*(\hat{\bm{k}}) Y_{lm}(\hat{\bm{r}}) e^{2i\sigma_l} \bar{S}_l
\end{align}
とした．

次に，式~\eqref{eq:c_plus_n_f}，~\eqref{eq:pure_c_asym}，を用いて~\eqref{eq:c_plus_n_aym}は，
\begin{align}
  \psi^{(+)}(\bm{k};\bm{r}) \rightarrow \frac{1}{(2\pi)^{3/2}}\qty[\frac{e^{-i\Theta(r)}}{r} Z_- + \frac{e^{i\Theta(r)}}{r} \tilde{Z}_+]
\end{align}
と書ける．ここで，
\begin{align}
  \tilde{Z}_+ = \bar{f}(\theta) + \frac{1}{2ik}\sum_{lm} 4\pi Y_{lm}^*(\hat{\bm{k}}) Y_{lm}(\hat{\bm{r}}) e^{2i\sigma_l}
\end{align}
である．ここで，
\begin{align*}
  \tilde{Z}_+ = \bar{Z}_+
...
から，
\begin{align}
  \bar{f}(\theta) &= \frac{1}{2ik} 4\pi Y_{lm}^*(\hat{\bm{k}}) Y_{lm}(\hat{\bm{r}}) e^{2i\sigma_l} \qty(\bar{S}_l - 1) \\
  &= \frac{1}{2ik}\sum_l (2l+1)P_l(\cos\theta) e^{2i\sigma_l} \qty(\bar{S}_l -1) \label{eq:c_plus_n_sa}
\end{align}
と分かる．
式~\eqref{eq:c_plus_n_aym}において，球面波の寄与は$\phi_C(\bm{k};\bm{r})$からもある．式~\eqref{eq:coulomb_sol_asym}，~\eqref{eq:c_plus_n_f}から
散乱振幅は
\begin{align}
  f(\theta) = f_C(\theta) + \bar{f}(\theta)
\end{align}
と書ける．よって微分散乱断面積は，
\begin{align}
  \dv{\sigma_C(\Omega)}{\Omega} = \abs{f_C(\theta) + \bar{f}(\theta)}
\end{align}
となる．
短距離の核力の影響を見るために，断面積をラザフォード散乱断面積との比で
規格化されることが多い．

クーロンポテンシャルのみの散乱では部分波展開が収束しなかったが，式~\eqref{eq:c_plus_n_sa}で書かれた無限和は収束する．$\bar{\delta}_l$は短距離ポテンシャル
の影響を受けなければ0となる．
よって，$r = R$より粒子が内側へと侵入する必要がある．
つまり，
\begin{align}
  \frac{l(l+1)}{R^2} + \frac{2k\eta}{R} < k^2
\end{align}
を満たす必要がある．これを満たす最大の$l$を$l_{\mathrm{cut}}$と置き，
$l_{\mathrm{cut}}(l_{\mathrm{cut}+1}) \approx l_{\mathrm{cut}}^2$
とし，
\begin{align}
  \frac{l_{\mathrm{cut}}(l_{\mathrm{cut}}+1)}{R^2} + \frac{2k\eta}{R} = k^2
\end{align}
を解くと，
\begin{align}
  l_{\mathrm{cut}} = ka \sqrt{1 - \frac{2\eta}{ka}}
\end{align}
を得る．同じ入射エネルギーにおいて$l > l_{\mathrm{cut}}$の部分波では，
$r = R$は古典的に禁止された領域であるため，その寄与は小さくなることがわかる．
よって，式~\eqref{eq:c_plus_n_sa}は収束することがわかる．


位相差$\bar{\delta}_l$の評価は，クーロンがない場合とまったく同様にして，
\begin{align}
  \tan \bar{\delta}_l = - \qty[\frac{k R F'_l(\eta, kR) - F_l(\eta, kR)\mathcal{L}^I}{kR G'_l(\eta, kR) - G_l(\eta, kR)\mathcal{L}^I}]
\end{align}
で得られる．ここで，$\mathcal{L}^I$は$r < R$の解からもとめた $r = R$における
動径波動関数の対数微分の値である．
また，$F_l，G_l$の微分は$\rho = kr$に対して行われる．




\section{同種粒子同士の散乱}
原子核散乱ではしばしば入射粒子と標的粒子が同種粒子であることがある．
この場合は，前節までで取り扱った結果に対していくつかの変更が加わる．
まず，重心系で$\hat{\bm{r}}$の方向にある検出器へ粒子が届く場合，
この粒子が入射粒子であるか標的粒子であるかを原理的に区別することができない．
運動量保存則から，$\hat{\bm{r}} \equiv (\theta，\varphi)$の方向へと粒子が
放出されたとき，もう片方の粒子は逆の方向へ，つまり$-\hat{\bm{r}} \equiv (\pi-\theta，\varphi + \pi)$
へと放出されるとわかる．よって，これら二つの方位における断面積は混ざりあうことがわかる．

量子力学において，全波動関数はボゾン同士の場合，二つの粒子の入れ替えに関して対称であり，
フェルミオンの場合は，反対称であることが課せられる．
スピンをもつ粒子の座標を$q = (\bm{r}， \mu)$とする．
ここで，$\bm{r}$が位置ベクトルであり，$\mu$がスピンの$z$成分である．
全波動関数が入れ替えの演算子，
\begin{align}
  P_{pt} = P^{\bm{r}}_{pt} P^{s}_{pt}
\end{align}
に関して固有状態である必要がある．
ここで，$P^{\bm{r}}_{pt}$は$\bm{r}_p$と$\bm{r}_t$の入れ替えの演算子であり，
$P^{s}_{pt}$は，$\mu_p$と$\mu_t$の入れ替えの演算子である．
つまり，
\begin{align}\label{eq:eigenstate_of_exchange_op}
  P_{pt}\psi_\lambda(q_p, q_t) = \psi_\lambda (q_t, q_p ) = \lambda \psi_\lambda(q_p, q_t)
\end{align}
である．ここで，ボゾン同士の場合$\lambda = +1$であり，フェルミオン同士の場合，$\lambda = -1$である．
式~(\ref{eq:eigenstate_of_exchange_op})を満たす波動関数は，対称化前の波動関数$\psi(q_t, q_p)$
を
用いることで，
\begin{align}
  \psi_+(q_p, q_t) = \frac{1}{\sqrt2}\qty[\psi(q_p, q_t) + \psi(q_t, q_p)] \\
  \psi_-(q_p, q_t) = \frac{1}{\sqrt2}\qty[\psi(q_p, q_t) - \psi(q_t, q_p)]
\end{align}
のように書くことができる．

最も簡単な場合として，スピンが入射粒子と標的核の入れ替えに関して対称な場合を考える．
これはスピン$s=0$の場合は成り立つが，$s \neq  0$では一般に成り立たない．
しかし，これはスピンが同じ方向に偏極した状況を考えると成り立つものである．
このとき，粒子の入れ替えの演算子は位置の入れ替えの演算子として書き換えることができるので，
波動関数は，
\begin{align}
  \psi_{\pm}(\bm{r}_p, \bm{r}_t) = \frac{1}{\sqrt{2}}\qty[\psi(\bm{r}_p, \bm{r}_t) \pm \psi(\bm{r}_t, \bm{r}_p)]
\end{align}
となる．ここで，$+$はボゾン同士，$-$はフェルミオン同士の場合である．

散乱理論において，位置ベクトル$\bm{r}_p$と$\bm{r}_t$を
重心位置ベクトル$\bm{R}_\text{c.m.}$と相対位置ベクトル$\bm{r}$で書くことがおおい．
これらは，
\begin{align}
  \bm{R}_\text{c.m.} = \frac{\bm{r}_p + \bm{r}_t}{2}，\quad \bm{r} = \bm{r}_p - \bm{r}_t
\end{align}
と書かれるため，位置の入れ替え変換の下で，
\begin{align}
  P^\dagger_{pt}\bm{R}_\text{c.m.} P_{pt} = \bm{R}_\text{c.m.}，\quad P^\dagger_{pt} \bm{r} P_{pt} = -\bm{r}
\end{align}
となることがわかる．よって，入れ替えの演算子は
\begin{align}
  P_{pt} = \Pi
\end{align}
とパリティ変換で書くことができる．
よって，重心系における全波動関数は，
\begin{align}
  \psi_{\pm} (\bm{r}) = \frac{1}{\sqrt{2}}\qty[\psi(\bm{r}) \pm \psi(-\bm{r})]
\end{align}
となる．入れ替えによる変換則から，全波動関数に関して，ボゾン同士の場合はパリティが正であること
，フェルミオン同士の場合はパリティが負であることが要請される．

全波動関数に関して，式~(\ref{eq:desired_wave_function})が変更されて，
\begin{align}
  \psi_{\pm}^{(+)} = \phi_{\pm} (\bm{r}) + \psi_{\pm}^{\text{sc}}(\bm{r}) 
\end{align}
となる．それぞれの漸近形は
\begin{align}
  \phi_{\pm}(\bm{r}) &\rightarrow \frac{1}{(2\pi)^{3/2}}\qty[\frac{e^{i\bm{k}_0 \cdot \bm{r}} \pm e^{-i\bm{k}_0 \cdot \bm{r}}}{\sqrt{2}}] \\
  \psi_{\pm}^{(\text{sc})}(\bm{r}) &\rightarrow \frac{1}{(2\pi)^{3/2}}\qty[\frac{f(\theta)\pm f(\pi - \theta)}{\sqrt{2}}]\frac{e^{ikr}}{r}
\end{align}
となる．
ここで，相互作用が球対称，つまり距離の大きさにのみよるという仮定のもと，散乱振幅$f$
が$\theta$にのみによるという事実を用いた．

逆向きに進む入射粒子が二つある（$e^{i\bm{k}_0 \cdot \bm{r}}$と$e^{-i \bm{k}_0 \cdot \bm{r}}$）．
散乱波解は両方の粒子の散乱を考慮している．この波動関数から計算される
フラックスは立体角$\dd{\Omega}$へ2つの粒子のどちらかが散乱される確率を与える．
断面積は，このフラックスを入射平面波のどちらか一方におけるフラックスで割った比として与えられる．
このことから，弾性散乱断面積は，
\begin{align}
  \dv{\sigma}{\Omega}_{\pm} = \abs{f(\theta) \pm f(\pi - \theta)}^2
\end{align}
と書くことができる．
\begin{align}
...
と書くことにすると，
\begin{align}
  \dv{\sigma}{\Omega}_{\pm} = \abs{f_{\pm}(\theta)}^2
\end{align}
と書ける．
この式から，単なる足し合わせではなく
干渉項
\begin{align}
  \pm 2 \mathrm{Re}\qty[f^*(\theta)f(\pi - \theta)]
\end{align}
という項も現れる．
これは量子力学特有の項であり，古典力学では現れない項である．

断面積は，$\theta = \pi//2$
に関して対称となっていることがわかる．
このことから，$f_{\pm}(\theta)$の部分波展開
について，ルジャンドル多項式$P_l$について，
\begin{align}
  P_l\qty(\cos (\pi - \theta)) = P_l(-\cos \theta) = (-1)^lP_l(\cos\theta)
\end{align}
という関係式を用いて，
\begin{align}
  f_{\pm}(\theta) = \frac{1}{2ik} \sum_l \qty[(2l+1)P_l(\cos\theta) (S_l - 1)]\times\qty[1 \pm (-1)^l]
\end{align}
と書くことができる．
この式から，ボゾンでは偶数の部分波，フェルミオンでは奇数の部分波のみが寄与することがわかる．
