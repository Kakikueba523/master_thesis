...
そうでないときhは，入射波動関数がポテンシャルによって歪められる．
...
\begin{align}
  \phi_C(\bm{k};\bm{r}) \sim e^{i(kz - \eta \ln k(r-z))} +  f_C(\theta) \frac{e^{ikr}}{r}\qty[e^{-i\eta \ln(2kr)}]
\end{align}
と漸近解が得られる．
...
クーロン波動関数$\phi_C(\bm{r};\bm{r})$を部分波展開すると，
\begin{align}
...
\begin{align}
  \sigma_l = \arg\qty[\Gamma(1+l + i\eta)]
\end{align}
を導入した．
