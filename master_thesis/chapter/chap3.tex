...
核子と原子核の散乱について考える．ただし，入射エネルギー$E$は，elastic channelのみがある場合
を考える．
まず，ハミルトニアンを
\begin{align}\label{eq:hamiltonian_internal_ex}
  H = H_0 + V + H'(\xi)
\end{align}
とする．ここで，$H_0$は運動項であり，$V$は入射核と標的核の相互作用である．
また，$H'(\xi)$は標的核の内部ハミルトニアンである．
ここで，
\begin{align}
  (E - H)\Psi = 0
\end{align}
というシュレーディンガー方程式について考える．
まず，$H'(\xi)$というハミルトニアンの固有状態
\begin{align}
  H'(\xi)\Phi_\alpha(\xi) = \varepsilon_\alpha \Phi_\alpha(\xi)
\end{align}
を用いて，波動関数$\Psi$を
\begin{align}
  \Psi = \sum_\alpha \psi_\alpha \Psi_\alpha(\xi)
\end{align}
と展開する．開いているチャネルが弾性チャネルのみである
場合，$\Psi$の弾性チャネルの部分は$\psi_0\Phi_0$
である．
ここで，開いているチャネルへの射影演算子$P$とそれ以外へのチャネルへの射影演算子$Q$
を導入する．
すなわち，
\begin{align}
  P\Psi = \psi_0 \Phi_0，\quad Q\Psi = (1 - P) \Psi
\end{align}
および，
\begin{align}
  P^2 \Psi = P \Psi，\quad Q^2 \Psi = Q \Psi，\quad QP \Psi = PQ\Psi = 0
\end{align}
を満たす演算子$P$および$Q$を導入する．
シュレーディンガー方程式~$(E-H)(P+Q)\Psi = 0$
に左から$P$と$Q$をそれぞれ掛けると，
\begin{align}
  (E - PHP)P\Psi = (PHQ)Q\Psi \label{eq:projected_to_P_space}\\
  (E - QHQ)Q\Psi = (QHP)P\Psi \label{eq:projected_to_Q_space}
\end{align}
を得る．



\subsection{クーロン場があるときの散乱理論}
