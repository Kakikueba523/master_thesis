...
ここでは，第\ref{chap:model_explanation}章で説明した模型による計算結果を載せてその内容について議論する．
...
\begin{align}\label{eq:modified_s_factor}
  S^*(E) = E \sigma(E) \exp(\frac{87.21}{\sqrt{E}} + 0.46 E).
\end{align}
以下では，modified $S$-factorのことを単に$S$-factorと呼ぶ．
...
\end{align}
を\isotope{C}+\isotope{C}チャネルに射影する．
このとき，有効ハミルトニアンとして，
\begin{align}
...
\begin{align}\label{eq:effctive_imaginary_part}
  \mathrm{Im} \qty(\bm{H}_{\mathrm{C+C，eff}}^{l, J^\pi})_{i,j} = \mathrm{Im}\sum_\mu \frac{v_0^2 \delta_{i,i_e}}{E - \mathcal{E}^{J^\pi}_\mu + i\Gamma^{J^\pi}(\mathcal{E}^{J^\pi}_\mu) /2} \delta_{ij}
\end{align}
となる．ここで，吸収断面積は
\begin{align}
  \sigma_{r} \propto w_J \int \dd{r} \mathrm{Im} \qty(\bm{H}_{\mathrm{C+C，eff}}^{l, J^\pi}) \abs{u(r)}^2
\end{align}
と書かれる．この値が\isotope[12]{C}+\isotope[12]{C}の共鳴エネルギー直上と，
\isotope[12]{C}+\isotope[13]{C}とで，一致させるように$v_0$の比を決める．
ただし，$w_J$はスピン統計因子であり，
\begin{align}
  w_J = (2J+1)\, \mathrm{for}\, \isotope[12]{C}+\isotope[12]{C}, \qquad w_J = \frac{2J+1}{2}\, \mathrm{for} \, \isotope[12]{C}+\isotope[13]{C}
\end{align}
である．
...
\begin{align}\label{eq:12_12_imaginary}
  \mathrm{Im} \qty(\bm{H}_{\mathrm{C+C，eff}}^{l, J^\pi}) = - i \frac{2v_0^2}{\Gamma^{J^\pi}(\mathcal{E}^{J^\pi}_C)} \delta_{i, i_e}
\end{align}
となる．

\isotope[12]{C}+\isotope[13]{C}の場合．
$E \sim E_C$を考える．ただし，$E_C$は複合核の固有エネルギーである．
準位間隔$D$と崩壊幅$\Gamma$を一定だと近似し，
\begin{align*}
...
\begin{align}
  \mathrm{Im} \qty(\bm{H}_{\mathrm{C+C，eff}}^{l, J^\pi}) = - v_0^2 \pi \rho^{J^\pi}(E_C)
\end{align}
である．ただし，$\rho = 1/D$である．

よって，スピン統計因子を含めた有効ポテンシャルの虚部の比が１となるようにすると，
\begin{align}\label{eq:strength_ratio_equation}
  (v_0^{12+12})^2 / (v_0^{12+13})^2 = \frac{\pi}{2}\left. \ev{ w_{J^{25}}\rho^{25}(E_C)} \middle/ \ev{w_{J^{24}}/ \Gamma^{24}(E_C)}\right.
\end{align}
と決めることができる．ただし，$\ev{w_{J^{24}}/\Gamma^{24}(E_C)} $は\isotope[24]{Mg}の崩壊幅の逆数とスピン統計因子の積のエネルギーおよびスピン平均であり，
$\ev{ w_{J^{25}}\rho^{25}(E_C)}$は\isotope[25]{Mg}の準位密度とスピン統計因子の積のエネルギーおよびスピン平均である．

今までの計算結果から，\isotope[12]{C}+\isotope[12]{C}で最も寄与する部分波は，
$l = 0，2$であり，\isotope[12]{C}+\isotope[13]{C}で最も寄与する部分波は，
$l = 0，1，2$であることがわかる．よって，これらの値における平均の値を比率の計算，式~(\ref{eq:strength_ratio_equation})
に用いる．すなわち，\isotope[24]{Mg}の$J = 0，2$における平均崩壊幅と，
\isotope[25]{Mg}の$J = 1/2, 3/2, 5/2$における平均準位密度を
2.0 MeV $< E_{\mathrm{c.m.}} <$ 5.5 MeV で計算する．
殻模型による計算と，遷移状態理論による見積りから，
\begin{align}
  \ev{w_{J^{24}}/\Gamma^{24}(E_C)} \simeq 24.1 \, \mathrm{MeV}^{-1} , \qquad  \ev{ w_{J^{25}}\rho^{25}(E_C)} \simeq 39.8 \, \mathrm{MeV}^{-1}
\end{align}
であるため，
これらの値をもとに計算する．
まず$v_0= 0.1，0.3，0.6$ MeV$\cdot$fm$^{1/2}$ を
\isotope[12]{C}+\isotope[13]{C}核融合反応に用いて，
\isotope[12]{C}+\isotope[12]{C}核融合反応には式~(\ref{eq:strength_ratio_equation})
を用いて変換した値を用いる．具体的には，
$v_0 = 0.16，0.48, 0.97$ MeV$\cdot$fm$^{1/2}$  を用いて計算した結果を
\begin{figure}[t]
  \centering
  \includegraphics[width= 0.9\textwidth]{figure/chap7/sfac_v0_0.15_comp.png}
  \caption{
    $v_0^{12+12} = 0.18$ MeV$\cdot$fm$^{1/2}$，$v_0^{12+13} = 0.1$ MeV$\cdot$fm$^{1/2}$，$a_r = 0.31$ fm としたときの$S$-factorの比較．
  }
...
\begin{figure}[t]
  ...
  \begin{subfigure}[t]{0.48\linewidth}
    ...
    \caption{$v_0^{12+12} = 0.54$ MeV$\cdot$fm$^{1/2}$，$v_0^{12+13} = 0.3$ MeV$\cdot$fm$^{1/2}$，$a_r = 0.31$ fm}
    \label{fig:v0_1212_0.85}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.48\linewidth}
    \centering
    \includegraphics[width=0.9\textwidth]{figure/chap7/sfac_v0_0.92_comp.png}
    \caption{$v_0^{12+12} = 1.1$ MeV$\cdot$fm$^{1/2}$，$v_0^{12+13} = 0.6$ MeV$\cdot$fm$^{1/2}$，$a_r = 0.31$ fm }
    \label{fig:v0_1212_1.7}
  \end{subfigure}
...
にプロットする．比率を調整しなかった場合に比べて，特に図~\ref{fig:v0_1212_1.7}を見れば
顕著であるが，\isotope[12]{C}+\isotope[13]{C}の
$S$-factorがよい上限を与えるようになったことが分かる．
...
\begin{figure}
  ...
  \caption{
    ...
    の値．星形のシンボルの位置が$\chi^2$の最小値であり，$a_r = 0.33$ fm，$v_0 = 0.41$ MeV fm$^{1/2}$ である．
  }
  \label{fig:chi2_plt}
\end{figure}
% \begin{table}[t]
...
\begin{figure}[tb]
  ...
  \caption{
    $a_r = 0.33$ fm とし，\isotope[12]{C}+\isotope[12]{C}系では$v_0 = 0.66$ MeV$\cdot$fm$^{1/2}$，\isotope[12]{C}+\isotope[13]{C}系では，
