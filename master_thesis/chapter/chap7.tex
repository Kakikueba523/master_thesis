...
\begin{align}\label{eq:strength_ratio_equation}
  (v_0^{12+12})^2 / (v_0^{12+13})^2 = \frac{\pi}{2}\ev{w_{J^{24}}\Gamma^{24}(E_C)} \ev{ w_{J^{25}}\rho^{25}(E_C)}
\end{align}
と決めることができる．ただし，$\ev{w_{J^{24}}\Gamma^{24}(E_C)} $は\isotope[24]{Mg}の崩壊幅とスピン統計因子の積のエネルギーおよびスピン平均であり，
...
$\ev{w_J \Gamma^{24}} \sim 0.051$ MeV であり，$\ev{w_J \rho^{25}} \sim 39.8$ MeV$^{-1}$ であるため，
これらの値をもとに計算する．
...
を用いて変換した値を用いる．具体的には，
$v_0 = 0.18，0.54, 1.1$ MeV$\cdot$fm$^{1/2}$  を用いて計算した結果を
\begin{figure}[t]
  \centering
